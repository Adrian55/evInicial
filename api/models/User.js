          ...
          if(profesor) {
            Profesor.update(profesor.id, {user: user.id}).then(function(profesores){
             Promise.bind({ }, User.findOne(user.id)
              ...
              .then(function (updatedUser) {
                sails.log.verbose('role "profesor" attached to user', profesor.nombre,profesor.apellidos);
                next();
              })
              .catch(function (e) {
                sails.log.error(e);
      		...
      		if(alumno) {
	      		Alumno.update(alumno.id, {user: user.id}).then(function(alumnos){
			       Promise.bind({ }, User.findOne(user.id)
			        ...
			        .then(function (updatedUser) {
			          sails.log.verbose('role "alumno" attached to user', alumno.nombre,alumno.apellido1,alumno.apellido2);
			          next();
			        })
			        .catch(function (e) {
			          sails.log.error(e);
